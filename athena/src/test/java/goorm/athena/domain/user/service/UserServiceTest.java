...
class UserServiceTest {
    ...
    private static Stream<Arguments> provideEmailsForExistenceCheck() {
        return Stream.of(
                Arguments.of("exists@example.com", true),
                Arguments.of("notexists@example.com", false)
        );
    }
    ...
    void 닉네임_최대_길이_유효성_검사_테스트() {
        UserCreateRequest invalidRequest = new UserCreateRequest(
                "valid@email.co12312321321321321123213132132132132132131231231231232131231232134213213213213123m", // 이메일은 유효한 값
                "pw",              // 비밀번호 길이 2 (최소 3) -> 실패
                "validNickname"    // 닉네임도 유효한 값
        );
